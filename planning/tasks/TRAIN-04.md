# TRAIN-04 — Полный training API audit

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: TRAIN-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Проверить обучаемость всех общих операторов независимо от возможности full-27B training в 128GB.

## Приёмка
Full-parameter toy training обязателен; full-27B memory limitation не маскируется CPU offload into same UMA.
...
Каталог `evidence/runs/train-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
