# MTP-02 — Exact speculative verification

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: MTP-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Реализовать acceptance/rejection с корректным sampling distribution; greedy и stochastic отдельно. Не путать heuristic draft с exact algorithm.

## Приёмка
Toy distribution tests и greedy equivalence; counters draft/accepted/rejected в отчёте.
...
Каталог `evidence/runs/mtp-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
