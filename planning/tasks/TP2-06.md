# TP2-06 — Поставить опциональный distributed component

**Фаза:** tp2_deferred · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** deferred

## Входные условия
Зависимости: TP2-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Отдельный extra/dependency path, отсутствие сети в default runtime, rollback two-node sessions.

## Приёмка
Single-Spark users install/use without distributed stack; approved TP2 release notes.
...
Каталог `evidence/runs/tp2-06/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
