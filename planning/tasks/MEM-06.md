# MEM-06 — Профиль UMA access patterns

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: MEM-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Малые reproducible измерения allocation/layout при одинаковой арифметике: GPU-hot weights, mixed CPU/GPU operations, cache reuse.

## Приёмка
Политика выбрана по correctness+замерам; zero-copy magic и числа bandwidth не обещаются без evidence.
...
Каталог `evidence/runs/mem-06/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
