# VIS-01 — Processor и visual token contract

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: QWN-09. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Pin preprocessing/resize/normalization/patching/special tokens без remote code; лимиты изображения и видео до decode.

## Приёмка
Golden tensors/token IDs, malformed/oversized media fail safely; no network fetch by default.
...
Каталог `evidence/runs/vis-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
