# GDN-01 — Сверить математический oracle и масштабы

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: QWN-01, CORE-06. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Проверить bundled NumPy recurrence/VJP против pinned independent PyTorch/reference. q/k normalization/readout scale outside primitive; scalar decay not log-decay.

## Приёмка
Finite-difference tests plus independent formula review; никаких compound-forward заявлений только по toy.
...
Каталог `evidence/runs/gdn-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
