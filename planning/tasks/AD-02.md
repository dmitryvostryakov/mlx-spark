# AD-02 — Forward-mode JVP

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: AD-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Реализовать tangents для общего operator inventory, проверять примитивы с несколькими выходами и shared parameters.

## Приёмка
JVP finite directional derivatives и сопоставление с VJP duality.
...
Каталог `evidence/runs/ad-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
