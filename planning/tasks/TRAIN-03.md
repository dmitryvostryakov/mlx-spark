...
## Входные условия
Зависимости: TRAIN-02, QTZ-03, GDN-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Подключить approved quantized base с точными input gradients, отключить MTP/vision training если ещё не validated; явные ограничения.

## Приёмка
QLoRA шагает на GB10 без OOM и без fake gradients; dtype map и peak memory приложены.
...
Каталог `evidence/runs/train-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
