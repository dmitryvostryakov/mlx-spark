# QWN-03 — Tokenizer, chat template и sampling contract

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: QWN-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Сохранить специальные токены, reasoning settings, BOS/EOS, tool-call formatting как формат данных. Pin processors.

## Приёмка
Exact token-ID golden fixtures на русском/английском/code; генерация сравнивается по одинаковым input IDs.
...
Каталог `evidence/runs/qwn-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
