# MEM-02 — Ввести явные allocation policies

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: MEM-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Определить correctness-first выбор HOST/SHARED/DEVICE и documented synchronization. CPU/GPU logical array API не означает все allocation types CPU-readable.

## Приёмка
CPU->GPU->CPU тесты values/lifetimes; отсутствие скрытых precision/device changes.
...
Каталог `evidence/runs/mem-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
