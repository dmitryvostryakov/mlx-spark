# GDN-03 — BF16 decode и numerical drift

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: GDN-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Добавить production dtype dispatch при FP32 state/accumulation; no hidden TF32/fp4 changes.

## Приёмка
Randomized q/k/v/g ranges, thousands-step recurrence drift и comparison before/after fusion.
...
Каталог `evidence/runs/gdn-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
