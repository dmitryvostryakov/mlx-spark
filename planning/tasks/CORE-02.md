# CORE-02 — Типы данных и продвижение типов

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: CORE-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Зафиксировать dtype promotion Python scalar/array, bool, integer overflow, FP16/BF16/FP32, complex и поддерживаемые wide types. Не молча сужать неподдерживаемый тип.

## Приёмка
Таблица dtype promotion сопоставлена с baseline; unsupported выдаёт согласованную ошибку.
...
Каталог `evidence/runs/core-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
