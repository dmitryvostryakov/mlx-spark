# NN-04 — Checkpoint/resume общего обучения

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: NN-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Параметры, optimizer, RNG, scheduler, scaler и dataset position; versioned metadata, atomic writes.

## Приёмка
Непрерывная N-step траектория совпадает с N/2+resume в установленном детерминированном режиме.
...
Каталог `evidence/runs/nn-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
