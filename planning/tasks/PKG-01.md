# PKG-01 — Завершить самостоятельный wheel/SDK

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: CORE-10, EXEC-06, MEM-07, AD-06, NN-06. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Own package and CMake exports; symbols/rpath/license/SBOM; minimal runtime deps, no installed mlx/torch imports.

## Приёмка
Wheel installation in fresh env, C++ example link/run, uninstall does not remove other packages.
...
Каталог `evidence/runs/pkg-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
