# FND-07 — Заморозить API inventory и совместимость
...
## Реализация
Извлечь точную поверхность API pinned MLX Python/C++ в изолированном oracle env. Сопоставить seed compatibility matrix, платформенные эквиваленты и deferred distributed. Не считать список примеров полным API.

## Приёмка
Каждый публичный символ имеет категорию, тест, статус и объяснение applicability; миграция namespace документирована.
...
Каталог `evidence/runs/fnd-07/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
