# QWN-06 — Полное внимание и positional semantics

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: QWN-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
GQA head ratio, attention output gate, causal mask, partial/multimodal RoPE. KV shape не выводить из hidden_size/heads.

## Приёмка
Prefill и cached decode совпадают; все position offsets корректны; длинные prompt тестируются постепенно.
...
Каталог `evidence/runs/qwn-06/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
