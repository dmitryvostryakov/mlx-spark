# EXEC-04 — CUDA Graph capture после корректной eager базы

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: EXEC-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Проверить допустимые allocations, адреса, state mutation, shape buckets и replay. Неподдерживаемое capture явно отключается, не меняя результат.

## Приёмка
Replay stateful decode корректен после cache growth и отмены; graph cache ограничен.
...
Каталог `evidence/runs/exec-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
