# GDN-06 — Профилирование и production dispatch

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: GDN-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Оптимизировать только measured hotspots: register pressure, occupancy, memory reads, launch count; rollback switch.

## Приёмка
End-to-end Qwen profile подтверждает пользу, operator fast path feature-gated по capabilities.
...
Каталог `evidence/runs/gdn-06/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
