# PERF-02 — Снять baseline одного Spark

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: PERF-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Baseline MLX CUDA отдельно от candidate, один GPU, ограниченный workload. Температура/питание где доступны, without changing clocks.

## Приёмка
Raw timings и memory, warm/cold разделены; p50/p95 and variability not single best run.
...
Каталог `evidence/runs/perf-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
