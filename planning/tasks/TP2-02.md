# TP2-02 — Инвентаризация QSFP и transport experiments

**Фаза:** tp2_deferred · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** deferred

## Входные условия
Зависимости: TP2-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Только в фазе 2 проверить link/MTU/addressing и NCCL/RDMA fallback ограничения Spark. Не предполагать GPUDirect or 256GB coherent UMA.

## Приёмка
Фактическая пропускная способность и latency measured; выбран поддерживаемый transport.
...
Каталог `evidence/runs/tp2-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
