# FND-04 — Собрать неизменённый upstream baseline изолированно
...
## Реализация
Собрать pinned MLX CUDA в отдельном prefix/venv только для baseline. Проверить актуальные CMake flags по pinned source, cuDNN и BLAS/LAPACK. Не заменять системный mlx и не делать pip install глобально.

## Приёмка
Сборка и минимальный array forward зафиксированы на GB10 либо точная воспроизводимая build-блокировка; baseline не назван MLX-Spark.
...
Каталог `evidence/runs/fnd-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
