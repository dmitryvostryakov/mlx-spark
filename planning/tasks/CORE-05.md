# CORE-05 — Matmul и свёртки

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: CORE-04. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Dense matmul, batched/broadcasted matmul, conv1d/2d/3d, groups, dilation и transpose conv на основе общего dispatch.

## Приёмка
Корректность contiguous/strided layouts, odd/tail shapes и всех режимов, используемых Qwen/vision.
...
Каталог `evidence/runs/core-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
