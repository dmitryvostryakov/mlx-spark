# EXEC-03 — Compile и ограничения специализации

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: EXEC-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Определить tracing, shape specialization/cache keys и invalidation по dtype/layout/toolchain. Не реализовывать compile как identity decorator.

## Приёмка
Есть измеримое fused/compiled исполнение и numerics parity; поддерживаемые dynamic-shape случаи явно проверены.
...
Каталог `evidence/runs/exec-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
