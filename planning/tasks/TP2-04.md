# TP2-04 — Collectives и запуск одной модели на двух узлах

**Фаза:** tp2_deferred · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** deferred

## Входные условия
Зависимости: TP2-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
TP=2 только; timeouts, failures, deterministic collectives where possible and rank logging. Не расширять до EP/PP.

## Приёмка
End-to-end forward and state transitions match TP1 in defined tolerance.
...
Каталог `evidence/runs/tp2-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
