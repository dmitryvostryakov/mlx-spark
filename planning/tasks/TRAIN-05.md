# TRAIN-05 — Документация обучающих режимов

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: TRAIN-04. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Рецепты LoRA/QLoRA с bounded budgets, recovery, reproducibility и объяснением unsupported full-model cases.

## Приёмка
Пользователь проходит рецепт из чистого env; документация соответствует real measured settings.
...
Каталог `evidence/runs/train-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
