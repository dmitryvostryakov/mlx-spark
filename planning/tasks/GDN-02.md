# GDN-02 — Скомпилировать и проверить CUDA reference

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: GDN-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Проверить bundled opt-in FP32 t=1 kernel на GB10: shape guards, masked state, tails, strides rejected; memcheck/racecheck.

## Приёмка
Реальный CUDA compile+execute+sanitizer отчёт; одна toy shape не закрывает operator coverage.
...
Каталог `evidence/runs/gdn-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
