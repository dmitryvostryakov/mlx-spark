# QWN-02 — Безопасный weight loader и manifest

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: QWN-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Читать safetensors index/headers, shapes/dtypes/total sizes, fail on unexpected/missing keys. Text-only исключения vision/MTP делать отчётом, никогда silently.

## Приёмка
Все tensors учтены; no pickle/trust_remote_code; bounded streaming; immutable input weights.
...
Каталог `evidence/runs/qwn-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
