# MEM-03 — Сделать stream-aware pool

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: MEM-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Reuse после завершения stream events, alignment, bounded caching, accounting active/reserved/pending. Не очищать live buffers.

## Приёмка
Stress разных streams, deferred free, view lifetime, repeated allocations и race tests.
...
Каталог `evidence/runs/mem-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
