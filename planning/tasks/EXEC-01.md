# EXEC-01 — Lazy graph и границы materialization
...
## Реализация
Сохранить динамическое построение, eval/async_eval и lifetime зависимостей. Метрики синхронизируются там, где обещают wall time.

## Приёмка
Операции до eval не притворяются выполненными; Python scalar/item trigger документирован.
...
Каталог `evidence/runs/exec-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
