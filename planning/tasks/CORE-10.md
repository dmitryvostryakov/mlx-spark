# CORE-10 — Аудит полноты общего ядра

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: CORE-09. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Обновить exact API inventory после реализации; проверить пользовательскую неизвестную архитектуру на общих primitives.

## Приёмка
Неиспользуемые Qwen операции также протестированы; parity release gate остаётся closed при незакрытых required символах.
...
Каталог `evidence/runs/core-10/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
