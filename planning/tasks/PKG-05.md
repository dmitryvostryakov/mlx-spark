# PKG-05 — Выпустить одноузловой release

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: PKG-04. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Заверить release notes/limitations/build manifest; upstream attribution, rollback and upgrade test. Package release locally, no public publish without authorization.

## Приёмка
Артефакты воспроизводимы, gates signed off independent reviewer; release file opens future TP2 planning only.
...
Каталог `evidence/runs/pkg-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
