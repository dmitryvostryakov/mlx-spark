# NN-06 — Смешанная точность и gradient accumulation

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: NN-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
BF16 политика, accumulation FP32, clipping, optional loss scaling где нужно, finite-gradient diagnostics.

## Приёмка
Accumulation эквивалентен большой effective batch в допусках; overflow обработан явно.
...
Каталог `evidence/runs/nn-06/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
