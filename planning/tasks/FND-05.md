# FND-05 — Импортировать проверенное ядро в собственную кодовую базу
...
## Реализация
Материализовать checked-in upstream-derived source с лицензией, отдельным commit происхождения и хешами. Создать собственный target/library; не строить продукт поверх внешнего installed mlx. Не выполнять слепой global sed.

## Приёмка
Происхождение каждого импортированного дерева прослеживается; проект собирается без исходного репозитория в соседнем каталоге.
...
Каталог `evidence/runs/fnd-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
