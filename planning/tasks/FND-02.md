# FND-02 — Инвентаризировать выбранный Spark без изменений системы

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: FND-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Запустить doctor на локальном выбранном Spark. Собрать архитектуру, DGX OS, toolkit, compiler, driver, доступную память и свободное место для build/cache. Не собирать токены, IP, UUID или процессы; не SSH на второй узел.

## Приёмка
Есть локальный обезличенный JSON; неизвестные параметры помечены unknown; согласованы безопасные бюджеты памяти и диска.
...
Каталог `evidence/runs/fnd-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
