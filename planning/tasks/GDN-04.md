# GDN-04 — Chunked prefill algorithm

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: GDN-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Выбрать алгоритм block/chunk recurrence с доказанной causal equivalence и ограниченным workspace. Serial oracle остаётся только тестом.

## Приёмка
Whole/chunk equivalence, varied lengths/nonzero state/masks; реально улучшенный prefill без decode regression.
...
Каталог `evidence/runs/gdn-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
