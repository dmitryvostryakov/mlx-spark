# AD-01 — Перенести reverse-mode и gradient contracts

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: CORE-06. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
VJP registration, multiple inputs/outputs, broadcasting reductions и unused gradients. Forward kernels без правил не должны выдавать zero gradients.

## Приёмка
Finite differences и базовые аналитические сравнения float64/float32 на маленьких формах.
...
Каталог `evidence/runs/ad-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
