# AD-03 — vmap и batching rules

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: AD-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Сохранить произвольные mapped axes, nested vmap и transform compositions без скрытого Python sequence loop как production implementation.

## Приёмка
vmap численно эквивалентен независимым примерам и корректно работает с grad.
...
Каталог `evidence/runs/ad-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
