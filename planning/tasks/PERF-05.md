# PERF-05 — Устойчивость и практические примеры

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: PERF-04. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Bounded long session and repeated training/inference, memory pressure, cancellation. Synthetic user tasks без external application.

## Приёмка
Soak logs, no leak/lockup, docs match real timings; energy only joules/token if actual telemetry adequate.
...
Каталог `evidence/runs/perf-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
