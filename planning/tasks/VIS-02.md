# VIS-02 — Vision encoder и projector

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: VIS-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Перенести encoder/projector из audited weights с shared ops; проверить norm/conv/attention precision.

## Приёмка
Layerwise numerics и simple image-text fixtures against oracle, memory report.
...
Каталог `evidence/runs/vis-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
