# AD-06 — Проверить optimized primitive transforms

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: AD-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Для каждого нового CUDA-kernel VJP/JVP/vmap либо safe semantic fallback, видимый в capabilities.

## Приёмка
Ни один optimized inference path не подменяет обучение неверным градиентом.
...
Каталог `evidence/runs/ad-06/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
