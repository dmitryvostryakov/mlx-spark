# PERF-01 — Заморозить benchmark protocol

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: QWN-09, EXEC-05, MEM-06. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Определить input IDs, prompts, revisions, precision, math mode, output length, context, concurrency, seed, warmup, synchronization, trials.

## Приёмка
Manifest валиден; baseline и candidate identical settings; no mixed quant/context comparison.
...
Каталог `evidence/runs/perf-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
