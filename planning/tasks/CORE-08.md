# CORE-08 — Linear algebra, FFT и остаток array API

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: CORE-07. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Закрыть непройденные категории pinned API, включая linalg, FFT, sort/scan, complex. Имеющиеся библиотеки использовать с корректным ownership.

## Приёмка
Ни одна общая API-категория не объявлена complete на основании только Qwen tests.
...
Каталог `evidence/runs/core-08/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
