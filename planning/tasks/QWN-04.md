# QWN-04 — Собрать текстовую модель из общих модулей

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: QWN-03, NN-02, CORE-06. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Соединить embeddings, 48 GDN/16 attention, FFN, norms и projection по audited config; generic primitives не знают имени Qwen.

## Приёмка
Intermediate activations и logits проверены послойно против independent reference; native core prerequisite перед execution.
...
Каталог `evidence/runs/qwn-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
