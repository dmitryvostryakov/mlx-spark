# EXEC-06 — Проверить композицию graph и обучения

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: EXEC-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
grad(compile), compile(grad), vmap/grad, checkpoint/recompute, RNG и state.

## Приёмка
Композиционные тесты проходят; unsupported case исключается явно, а не detach.
...
Каталог `evidence/runs/exec-06/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
