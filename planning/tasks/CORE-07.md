# CORE-07 — Random и явное состояние RNG
...
## Реализация
Определить seed/key, split, reproducibility в eager/compile и save-resume. Не обещать bitwise CPU/GPU там, где алгоритмы различны.

## Приёмка
Воспроизводимость в пределах одного согласованного backend/config и формальные межплатформенные допуски.
...
Каталог `evidence/runs/core-07/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
