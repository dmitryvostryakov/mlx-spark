# QWN-05 — Согласовать GDN и convolution state

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: QWN-04, MEM-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Раздельно recurrent FP32, convolution history, attention KV, positions, RNG. Версионировать snapshot и проверить exact state transitions.

## Приёмка
Chunked/whole prefill, arbitrary boundaries и branch/resume эквивалентны; masked tokens не двигают state.
...
Каталог `evidence/runs/qwn-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
