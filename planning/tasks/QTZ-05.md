# QTZ-05 — Дополнительные precision formats по evidence

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: QTZ-04. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Рассмотреть FP8/NVFP4 только после первого стабильного format; проверить hardware/packing/library path.

## Приёмка
Каждый новый формат самостоятельная capability с tests; нет рекламного списка непроверенных precisions.
...
Каталог `evidence/runs/qtz-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
