# AD-04 — Higher-order differentiation

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: AD-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Реализовать/проверить вторые производные и non-smooth conventions pinned MLX.

## Приёмка
Hessian-vector toy test, grad-of-grad; отсутствие silent no-backward paths.
...
Каталог `evidence/runs/ad-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
