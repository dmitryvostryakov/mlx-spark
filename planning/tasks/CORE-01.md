# CORE-01 — Массивы: владение, scalar и shape

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: FND-06. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Перенести array ownership, shape/strides, contiguity, scalar и zero-sized semantics. Разграничить logical values и physical allocation. Проверить ошибки overflow и lifetime.

## Приёмка
Набор property tests для scalar, empty, views, aliasing и out-of-bounds проходит на CPU и CUDA.
...
Каталог `evidence/runs/core-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
