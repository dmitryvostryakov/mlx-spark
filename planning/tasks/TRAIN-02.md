# TRAIN-02 — LoRA checkpoint resume и reproducibility

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: TRAIN-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Сохранять optimizer/scheduler/RNG/dataloader position, gradient accumulation и adapter manifest.

## Приёмка
Split/resume trajectory в tolerance; checksum base weights до/после совпадает.
...
Каталог `evidence/runs/train-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
