# NN-02 — Layers, initializers и losses

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: NN-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Портировать общие слои и losses pinned surface; loss reduction axes и ignored labels.

## Приёмка
Toy MLP/conv/attention обучаются через публичное nn API.
...
Каталог `evidence/runs/nn-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
