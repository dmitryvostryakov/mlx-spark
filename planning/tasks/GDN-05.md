# GDN-05 — Backward и transform интеграция

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: GDN-04. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
VJP optimized path с checkpoint/recompute tradeoff; JVP/vmap/higher-order fallback к differentiable primitives where valid.

## Приёмка
Analytic+finite differences+Qwen LoRA gradients; no silent detach at chunk boundary.
...
Каталог `evidence/runs/gdn-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
