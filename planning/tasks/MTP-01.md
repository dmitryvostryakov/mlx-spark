# MTP-01 — Аудит MTP weights и алгоритма

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: QWN-09, GDN-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Проверить dedicated embeddings, hidden state plumbing и spec definition; loaded MTP tensors нельзя discard.

## Приёмка
Manifest accounted, t=1 draft/reference output сравнен; unsupported template явно ошибкой.
...
Каталог `evidence/runs/mtp-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
