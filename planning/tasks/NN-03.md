# NN-03 — Optimizers и schedules

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: NN-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
SGD, Adam/AdamW и остальная обязательная поверхность; state dtype, schedule step, weight decay exclusions.

## Приёмка
Сопоставление шагов с независимым oracle; mixed precision не теряет master/moment state.
...
Каталог `evidence/runs/nn-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
