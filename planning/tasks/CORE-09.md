# CORE-09 — Interoperability и export

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: CORE-08. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
DLPack/NumPy bridge, CPU/device visibility, stream synchronization, export/import supported functions. Не отдавать host-доступ к невидимому device pointer.

## Приёмка
Ownership, zero-copy/copy disclosure и producer/consumer lifetime tests; нет dangling capsule.
...
Каталог `evidence/runs/core-09/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
