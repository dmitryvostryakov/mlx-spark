# QWN-07 — Генерация и lifecycle

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: QWN-06. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Streaming tokens, max_new_tokens, EOS, cancellation, error cleanup, deterministic greedy reference и controlled sampling.

## Приёмка
Session A/B isolation, cancellation recovery, no stale cache; текстовая CLI без HTTP-serving.
...
Каталог `evidence/runs/qwn-07/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
