# PKG-03 — Документация Apple-like workflow

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: PKG-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Array/grad/compile/train/save/load/quantize/Qwen/diagnostics and troubleshooting. Все examples executable и status tagged.

## Приёмка
Fresh-user path проходит без ручных правок скрытых env vars; не пять несовместимых APIs.
...
Каталог `evidence/runs/pkg-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
