# TRAIN-01 — LoRA BF16 end-to-end

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: NN-06, QWN-09. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Выбрать audited target modules, adapter dtype/rank и optimizer, dataset local synthetic. Проверить gradients through recurrent stack.

## Приёмка
Train loss curve + held-out sanity; frozen base unchanged; adapter restore даёт тот же forward.
...
Каталог `evidence/runs/train-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
