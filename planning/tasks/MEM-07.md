# MEM-07 — Долговременный memory stress

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: MEM-06. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Повторная загрузка/выгрузка, cancellation, train/infer transitions, долгие stateful сессии. Ограничить нагрузку явно.

## Приёмка
Временной ряд active/reserved/RSS и high-water marks; нет монотонного необъяснённого роста.
...
Каталог `evidence/runs/mem-07/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
