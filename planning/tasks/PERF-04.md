# PERF-04 — Сравнить candidate с baseline

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: PERF-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Same token IDs/precision/context; model time vs tokenize/load отдельно; GPU sync for elapsed time.

## Приёмка
Paired deltas с variability и нет необъяснённой существенной регрессии; CPU oracle timing не называется GPU performance.
...
Каталог `evidence/runs/perf-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
