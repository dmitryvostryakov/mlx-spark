# QTZ-02 — Converter с ограниченной памятью

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: QTZ-01. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Streaming конвертация, source/output разные пути, неизменённые norms/gates/embeddings по явной карте.

## Приёмка
Все веса accounted; metadata сохраняет revision и calibration; peak memory/disk измерены.
...
Каталог `evidence/runs/qtz-02/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
