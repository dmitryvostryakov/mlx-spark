# VIS-03 — Multimodal RoPE и интеграция

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: VIS-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Positions/spatial merge/temporal patches, image tokens в language stack, chunking constraints.

## Приёмка
Text-only поведение не регрессирует; mixed text/image batch state correct.
...
Каталог `evidence/runs/vis-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
