# VIS-04 — Видео и мультимодальная приёмка

**Фаза:** single_spark · **Владелец:** Luna · **Reviewer:** Sol · **Статус:** todo

## Входные условия
Зависимости: VIS-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Ограниченные local synthetic video fixtures, frame sampling, budgeting и deterministic processor settings.

## Приёмка
Image+video tests проверены; безопасность декодера/лимиты задокументированы; no unsupported full-VLM claim.
...
Каталог `evidence/runs/vis-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
