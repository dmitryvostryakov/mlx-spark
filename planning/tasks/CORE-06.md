# CORE-06 — Нормы и attention primitives

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: CORE-05. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Общие RMSNorm/LayerNorm, softmax/logsumexp, attention mask и GQA. No hidden q/k scale. Unit-offset norms только по модели.

## Приёмка
Численные эталоны и supported shape/dtype inventory; совпадение при больших отрицательных logits.
...
Каталог `evidence/runs/core-06/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
