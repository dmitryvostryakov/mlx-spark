# MTP-03 — Откат полного hybrid state

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: MTP-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Snapshot/replay/rollback включает recurrence, conv, KV, positions, RNG. Усечение KV недостаточно.

## Приёмка
Reject at every draft position, cancellation and branching fixtures match non-MTP continuation.
...
Каталог `evidence/runs/mtp-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
