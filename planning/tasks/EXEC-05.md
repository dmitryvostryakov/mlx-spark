# EXEC-05 — Profiler hooks и numerical mode

**Фаза:** single_spark · **Владелец:** Sol · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: EXEC-04. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Добавить осмысленные operator/model ranges, sync timing, deterministic mode, TF32/fast-math/flush policy.

## Приёмка
Отчёт показывает реальный backend, выбранный kernel и math policy; fast paths имеют kill switch.
...
Каталог `evidence/runs/exec-05/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
