# QTZ-04 — Quality и performance comparison

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: QTZ-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Сопоставить BF16 и первый quant на fixed holdout, NLL/logits/task metrics, длинных состояниях.

## Приёмка
Quality thresholds preregistered/approved; held-out данные не используются для calibration; regression причины объяснены.
...
Каталог `evidence/runs/qtz-04/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
