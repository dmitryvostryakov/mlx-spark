# QTZ-03 — Quantized matmul и training compatibility

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: QTZ-02. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Weight-only quantized forward, dequantization reference, exact backward к inputs/LoRA; integer weights не получают фиктивные gradients.

## Приёмка
Matrix/tail shapes и dtype проверены; frozen base + LoRA дают верные gradients.
...
Каталог `evidence/runs/qtz-03/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
