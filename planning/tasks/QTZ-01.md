# QTZ-01 — Зафиксировать первый quant format

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: QWN-09, GDN-03. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Выбрать один воспроизводимый groupwise affine 4-bit format или иной измеренно обоснованный; документировать packing, scales, zero/bias, axes, exclusions.

## Приёмка
Формат round-trip и forward reference определён; NVFP4/INT4 не смешиваются как синонимы.
...
Каталог `evidence/runs/qtz-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
