# MEM-01 — Проверить реальные memory capabilities GB10

**Фаза:** single_spark · **Владелец:** Terra · **Reviewer:** Luna · **Статус:** todo

## Входные условия
Зависимости: FND-04. Статус зависимости `done` действителен только с evidence и review.
Обязательный контекст: `AGENTS.md`, `CONTEXT.md`, `handoff/AGENT_PROTOCOL.md`, применимая спецификация из `docs/`.

## Реализация
Скомпилировать capability probe и минимальные visibility-тесты. Сопоставить cudaMalloc, host/pinned, managed и supported pageable доступы; не полагаться только на integrated flag.

## Приёмка
Матрица capability/visibility с доказательствами и доступными allocation types; никаких выводов по nvidia-smi alone.
...
Каталог `evidence/runs/mem-01/`: `run.json` с revision/toolchain/hardware/командами; неизменённые stdout/stderr; тестовый отчёт; численные результаты, если применимо; review note.
